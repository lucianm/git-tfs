...
namespace GitTfs.Test.Util
{
    public class GitTfsCommandRunnerTests : BaseTest
    {
        ...
        private readonly GitTfsCommandRunner _classUnderTest;

        public GitTfsCommandRunnerTests()
        {
            _mocks = new AutoMocker();
            _classUnderTest = _mocks.CreateInstance<GitTfsCommandRunner>();
        }
        ...
        [Fact]
        public void ReturnsCommandReturnValue() => Assert.Equal(99, _classUnderTest.Run(new UsesList(), Args()));
        ...
        public void CallsListWithZeroArgs()
        {
            ...
            var args = Args();
            _classUnderTest.Run(command, args);
        ...
        {
            _mocks.GetMock<IHelpHelper>().Setup(x => x.ShowHelpForInvalidArguments(It.IsAny<GitTfsCommand>())).Returns(33);
            Assert.Equal(33, _classUnderTest.Run(new UsesOverloads(), Args()));
        }
        ...
        {
            _mocks.GetMock<IHelpHelper>().Setup(x => x.ShowHelpForInvalidArguments(It.IsAny<GitTfsCommand>())).Returns(33);
            Assert.Equal(33, _classUnderTest.Run(new UsesOverloads(), Args("a", "b", "c")));
        }
